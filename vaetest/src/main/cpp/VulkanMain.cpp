...
void CreateTexture(void) {
  for (uint32_t i = 0; i < TUTORIAL_TEXTURE_COUNT; i++) {
    ...
    std::vector<char> fileContent = FileUtils::getBufferFromFile(androidAppCtx, texFiles[i]);
    ...
    textures[i]->CreateTexture(imageData,imgWidth,imgHeight, usage,required_props);
...
bool InitVulkan(android_app* app) {
  ...
  LOGI("Create Texture");
  CreateTexture();
  material.vertString = FileUtils::getBufferFromFile(androidAppCtx, "shaders/tri.vert");
  material.fragString = FileUtils::getBufferFromFile(androidAppCtx, "shaders/tri.frag");
...
bool IsVulkanReady(void) { return renderer && renderer->isReady(); }
